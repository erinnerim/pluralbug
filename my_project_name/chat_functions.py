...
async def set_displayname(
    ...
    if not (display_name in users):
        users[display_name] = None
        write_to_csv(users, 'users.csv')
    return await client.set_displayname(display_name)
...
async def set_pfp(
    ...
    new_pfp = ""
    users = read_from_csv('users.csv')
    if (img_path is None): # image path not specified
        if not (name in users): # user not exists already
            users[name] = None
        if not (users[name] is None): # pfp exists for user
            new_pfp = users[name]
    else: # image path specified
        print("Uploading file: " + img_path)
        async with aiofiles.open(img_path, "r+b") as f:
        ...
        users[name] = resp.content_uri
        new_pfp = resp.content_uri
        await client.room_redact(room_id, resp.uuid)
    print(users)
    write_to_csv(users, 'users.csv')
    await client.set_avatar(new_pfp)
...
async def delete_event(
    ...
    print("delete this event: " + event_id)
